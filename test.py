...
def test(args, data_loader, model):
    ...
    for i, (text, label) in enumerate(tqdm(data_loader)):
        text = text.T
        input_lengths = torch.LongTensor([torch.max(text[i, :].data.nonzero())+1 for i in range(text.size(0))]) 
        ...
        output = predicted_label.argmax(dim=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vocab_size', type=int, default=50000, help="maximum vocab size")
